...
combined_data <- covid_data %>%
  ...
  mutate(temp_from_median = abs(temp - 22)) %>% 
  select(-value)
...
combined_data %>% 
  ...
  labs(title = "Orleans Parish", subtitle = " Correlation Between Covid Cases and Temperature") +

  # ggtitle("Covid Cases vs Temperature in Orleans Parish") +
...
```{r}
data <- get_county_data("22071")
```

```{r}
plot_county_data(data)
```
