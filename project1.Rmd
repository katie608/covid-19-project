...
get_weather_data <- 
  function(FIPS) {
    ...
    datatypeid='TAVG', # can be PRCP (precipitation), TMAX,      TMIN, or TAVG(sometimes)
...
# ncdc_plot(out, breaks="1 month", dateformat="%m")
...
```{r}
county_fips = '22071'
weather_data <- get_weather_data(county_fips)$data %>% 
  mutate(date = as.Date(date)) 
covid_data <- df_daily %>% filter(fips == county_fips)
```
...
combined_data %>% 
  ggplot(aes(x = date)) +
  geom_point(mapping = aes(y = value, color = "Temperature")) +
  geom_smooth(mapping = aes(y = value, color = "Temperature"))+
  geom_point(mapping = aes(y = daily_case_count, color = "Daily Cases")) 
  # geom_smooth() 
